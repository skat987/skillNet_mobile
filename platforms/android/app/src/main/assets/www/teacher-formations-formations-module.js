    ...
    FormationsPage.prototype.setFormations = function () {
        ...
        this.apiService.get('teacher/myFormations').then(function (resp) {
            for (var i = 0; i < resp['data'].length; i++) {
                // tslint:disable-next-line:max-line-length
                _this.teacher.addFormation(new _models_formation__WEBPACK_IMPORTED_MODULE_5__["Formation"](resp['data'][i].id, resp['data'][i].name, resp['data'][i].logo, resp['data'][i].start_at, resp['data'][i].end_at, resp['data'][i].total_students));
